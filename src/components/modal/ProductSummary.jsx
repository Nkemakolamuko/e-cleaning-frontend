...
const ProductSummary = () => {
  ...
  useEffect(() => {
    ...
    const sumTotal = totalPrice.reduce((pVal, cVal) => {
      return Number(pVal) + Number(cVal);
    }, 0); // The initial value of '0' is so it doesn't through an error when the array is empty (Probably when we deleting items from cart) - (this is why the total goes back to initial value before summing up again)
    // Then of course I set the value to an existing null 'total'
    setTotal(sumTotal);
  }, [cartItem]);
